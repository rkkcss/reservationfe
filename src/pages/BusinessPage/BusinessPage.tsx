...
import { PublicReservationProvider, usePublicReservation } from '../../context/PublicAppointmentReservation'
...
export default function BusinessPage() {
    ...
    useEffect(() => {
        if (tenantSlug) {
            getBusinessBySlugQuery(tenantSlug)
    ...
    const handleValuesChange = (changedValues: Record<string, unknown>) => {
        ...
        if ("employeeId" in changedValues) {
            ...
            console.log("form values after employeeId reset:", form.getFieldsValue());
        }
            ...
            <div className="min-h-dvh overflow-x-hidden">
                <section className="relative h-[550px] w-full">
                    <div className="!absolute !inset-0 w-full h-full">
                        <Image alt="Interior" width={"100vw"} height={"100%"} src="https://images.unsplash.com/photo-1557683316-973673baf926?auto=format&fit=crop&w=1200&h=400&q=80" />
                    </div>
                    ...
                    <div className="relative h-full container mx-auto pt-16 flex flex-col items-center text-center text-white">
                        <div className="flex items-center gap-2 mb-4 bg-black/20 backdrop-blur-md px-4 py-1 rounded-full border border-white/20">
                            <img alt="Logo" className="h-5 w-5 invert" src="https://lh3.googleusercontent.com/aida/AP1WRLuPrfLaZswjHQRGn7SD5IneQ6Cq9etfbgt4CswvZc6qMyS35IGAn1hA1xBBPJ_3NChkKWk0B-OBJ7ZjD-tDVK89pmai3AS7eg8-UnzhaZsCM9bYepBvZu-pULJ3_1YFLt8_-lMFuUBVWMVS-b6GQIn3FGbWfg9_f9zngsFsYNXyHhxmygumIq7U1K8rE9k8aoFho6xMN_PN8NkHTUeY8qMSapEqRgPrNTpCaeZw-BETRFinAezn6VPTLss" />
                            <span className="text-xs font-bold uppercase tracking-[0.2em]">{business?.name}</span>
                        </div>
                        <Typography.Title level={1} className="!text-[48px] leading-tight mb-4 drop-shadow-2xl">{business?.description}</Typography.Title>
                ...
                <div className="max-w-screen-xl mx-auto px-16 -mt-40 relative z-10 w-full mb-12">
                    <Form form={form}
                    ...
                    >
                        <div className="bg-white rounded-[2rem] shadow-2xl border border-outline-variant overflow-hidden flex flex-col md:flex-row">
                            <div className="flex-1 p-10 border-b md:border-b-0 md:border-r border-outline-variant space-y-10">
                                <Form.Item
                                    name="employeeId"
                                    className="space-y-4"
                                    rules={[{ required: true, message: 'Kérjük válasszon szakembert!' }]}
                                    label={
                                        <p className="text-xs font-bold uppercase tracking-widest">Szakember Kiválósága</p>
                                    }
                                >
                                ...
                                <div className="space-y-4">
                                    <Form.Item
                                        name="offering"
                                        label={<p className="text-xs font-bold uppercase tracking-widest">Szolgáltatás kiválasztása</p>}
                            ...
                            <div className="w-full md:w-[420px] bg-surface-container-lowest p-10 flex flex-col space-y-8">
                                <div className="space-y-2">
                                    <p className="text-xs font-bold uppercase tracking-widest">Időpont</p>
