...
export const useAppointmentForm = (
    appointment: Appointment | CreateAdminAppointmentRequest | undefined,
    open: boolean,
    onClose: () => void,
    onOk?: (values: CreateAdminAppointmentRequest) => void,
) => {
        ...
        if (appointment?.businessEmployee) {
            setEmployeeOptions((prev) => {
                const exists = prev.some(
                    (item) => item.id === appointment.businessEmployee?.id,
                );
